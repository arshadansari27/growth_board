...
def update_from_hiring_board():
    ...
    def setup(_task, board):
        ...
        task_row.link = _task.get_browseable_url()
        if getattr(_task, 'Schedule', None) and isinstance(_task.Schedule, NotionDate):
            task_row.scheduled = NotionDate(_task.Schedule.start, _task.Schedule.end, DEFAULT_TIMEZONE)
